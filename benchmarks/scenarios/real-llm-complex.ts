// ================================================================
//  Scenario: Real LLM Complex Benchmark
//  Claim 8 — Evolution improves on diverse, complex tasks
//
//  Uses auto-detected adapter with ComplexEvaluator (4 task types):
//  word-problem, logic, json-extraction, pattern-completion.
//  Compares ecology (evolution) vs static baseline over 10 cycles.
//  Minimizes API cost: 5 strategies × 10 cycles ≈ 50 calls + baseline.
// ================================================================

import 'dotenv/config';
import { Ecology } from '../../src/evolution/ecology.js';
import { ComplexEvaluator } from '../evaluators/complex-evaluator.js';
...
export async function realLlmComplex(
  seed: number,
  cycles = 10,
): Promise<BenchmarkResult> {
  return runBenchmark('real-llm-complex', seed, async (s) => {
    ...
    console.log(`  [real-llm-complex] Using ${adapterInfo.name} (${adapterInfo.model})`);

    // ── Ecology (evolution) ──────────────────────────────────────
    resetGenomeCounter();
    const ecologyEval = new ComplexEvaluator(rng1);
    let ecologyTokens = 0;
    ...
    // ── Static baseline ──────────────────────────────────────────
    resetGenomeCounter();
    const staticEval = new ComplexEvaluator(rng2);
    let staticTokens = 0;
    ...
    return {
      ...
      timeSeries,
      details: passed
        ? `Complex tasks fitness improved: ${earlyAvgFitness.toFixed(2)} → ${lateAvgFitness.toFixed(2)} (+${(improvementRatio * 100).toFixed(0)}%), ecology best=${ecologyFinalBest.toFixed(2)} vs static=${staticFinalBest.toFixed(2)}, tokens: ${ecologyTokens}+${staticTokens}`
        : `Complex tasks fitness did NOT improve: early=${earlyAvgFitness.toFixed(2)}, late=${lateAvgFitness.toFixed(2)}, tokens: ${ecologyTokens}+${staticTokens}`,
    };
  });
}
